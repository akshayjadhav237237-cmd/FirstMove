...
export default function Screen2_Questions({ isLoading }) {
  ...
  return (
    <div className="scrollable-screen w-full flex-1 flex flex-col lg:flex-row min-h-screen relative p-1 pb-4">
      ...
      <div className="w-full lg:w-[44%] flex flex-col glass-spatial p-8 m-4 lg:my-4 lg:mr-2 lg:ml-4 overflow-y-auto max-h-[calc(100vh-32px)]">
        
        {/* Step Pill */}
        <div className="self-start inline-flex items-center rounded-full bg-white/5 border border-white/10 px-3.5 py-1.5 text-xs text-white/70 font-mono tracking-wider font-semibold mb-6">
          Step 2 of 3
        </div>

        {/* Heading */}
        <h1 className="text-3xl font-extrabold text-white tracking-tight mb-1" style={{ letterSpacing: "-0.025em" }}>
          A few questions
        </h1>
        ...
        <div className="flex-1 space-y-6">
          {isLoading ? (
            <div className="space-y-4">
              <p className="text-white/60 text-[10px] font-mono uppercase tracking-widest animate-pulse">
            ...
            state.socraticQuestions.map((q, i) => (
              <div key={q.id} className="flex flex-col">
                ...
                <textarea
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl p-3.5 text-white text-sm min-h-[80px] resize-none placeholder:text-white/15 outline-none focus:bg-white/10 transition-all duration-200"
                />
              </div>
            ))
          )}
        ...
        <div className="pt-6 border-t border-white/10 mt-6 flex flex-col gap-4">
          ...
          <div className="flex justify-between items-center">
            <button
              ...
              className="text-white/35 hover:text-white text-[10px] font-mono tracking-wider font-semibold transition-colors cursor-pointer uppercase"
            >
              ← Restart
            </button>
          </div>
          ...
          <button
            ...
            className="btn-spatial-pill w-full py-4 text-black font-extrabold text-sm tracking-wider disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer flex items-center justify-center gap-2 uppercase transition-all duration-200"
          >
            {isLoading ? (
              <>
                <span className="w-4 h-4 border-2 border-black border-t-black rounded-full animate-spin" />
      ...
      <div className="w-full lg:w-[56%] flex flex-col m-4 lg:my-4 lg:ml-2 lg:mr-4 max-h-[calc(100vh-32px)]">
        ...
        <div className="mt-8 mb-4 pl-2">
          <span className="mono-label text-xs tracking-widest text-white/50 font-semibold">
            Agents standing by
          </span>
        </div>
        ...
        <div className="flex-1 space-y-4 overflow-y-auto pr-1">
          {AGENTS.map((agent) => (
            <div key={agent.key} className="glass-spatial p-5 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-4 flex flex-col gap-1.5">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className={`w-1.5 h-1.5 rounded-full bg-white ${isLoading ? "animate-ping" : "animate-pulse"}`} style={{ boxShadow: "0 0 8px #fff" }} />
